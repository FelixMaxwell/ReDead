...
function meta:SetPlayerClass( name )
	self.Class = name
end

function meta:GetPlayerClass()
	return self.Class or GAMEMODE.Classes.Human.Default
end
...
function meta:OnSpawn()
	...
	if self:Team() == TEAM_ARMY then
		...
		self:SetCash( GAMEMODE.Classes.Human[self:GetPlayerClass()].cash or 15)
		...
		self:SetMaxHealth( GAMEMODE.Classes.Zombie[ self:GetPlayerClass() ].health )
		self:SetHealth( GAMEMODE.Classes.Zombie[ self:GetPlayerClass() ].health )
...
function meta:GetItemLoadout()
	...
	return GAMEMODE.Classes.Human[ self:GetPlayerClass() ].loadout
...
function meta:OnLoadout() // this code is terrible, i just cant be arsed to tidy it up
	...
	if self:Team() == TEAM_ARMY then
		...
		for k, v in pairs( GAMEMODE.Classes.Human[ self:GetPlayerClass() ].weapons ) do
		...
		for k,v in pairs( GAMEMODE.Classes.Zombie[ self:GetPlayerClass() ].weapons ) do
			self:Give( v )
		end
	end
...
function meta:Think()
	...
	if ( self.StamTime or 0 ) < CurTime() then 
		...
		elseif self:GetRadiation() < 1 then
			...
			if (GAMEMODE.Classes.Human[self:GetPlayerClass()].staminatime or 0) > 0 then
				
				self.StamTime = CurTime() + GAMEMODE.Classes.Human[self:GetPlayerClass()].staminatime
...
function meta:OnDeath()
	...
	if self:Team() == TEAM_ARMY then
		...
		self:SetCash( 0 )
		self:Flashlight( false )
		self:SetPlayerClass( GAMEMODE.Classes.Zombie.Default )
		self.Inventory = {}
		self:SynchInventory()
		...
		if (GAMEMODE.Classes.Zombie[self:GetPlayerClass()].splash or false) then
