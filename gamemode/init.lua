...
AddCSLuaFile( 'vgui/vgui_classpickerbutton.lua' )
AddCSLuaFile( 'newclasses.lua' )
...
function GM:PickLord( force )
	...
	if ply:Team() == TEAM_ZOMBIES then
		...
		timer.Simple( 3, function() ply:SetTeam( TEAM_ZOMBIES ) ply:SetPlayerClass( GAMEMODE.Classes.Human.Default ) ply:SetCash( 0 ) ply:Gib() ply:SetLord( true ) end )
...
function GM:EntityTakeDamage( ent, dmginfo )
	...
	if ent:IsPlayer() and ent:Team() == TEAM_ARMY and IsValid( attacker ) and ( attacker:IsNPC() or ( ( attacker:IsPlayer() and attacker:Team() == TEAM_ZOMBIES ) or ( attacker:IsPlayer() and attacker == ent ) ) ) then
		...
		dmginfo:ScaleDamage( GetConVar( "sv_redead_dmg_scale" ):GetFloat() * (GAMEMODE.Classes.Human[ent:GetPlayerClass()].damagescale or 1))
...
function GM:ShowTeam( ply )
	...
	if ply:IsIndoors() then
		...
		if (GAMEMODE.Classes.Human[ply:GetPlayerClass()].sptrader or false) then
